...
render_segment_face_image (gl, ofstx, ofsty, texw, texh, detection, face_id, bisenetv2_ret)
{
    ...
    let color = [
        ...
        0,   128, 0,   alpha,  /* [2 ] l_brow */
        128, 128, 0,   alpha,  /* [3 ] r_brow */
        0,   0,   128, alpha,  /* [4 ] l_eye */
        128, 0,   128, alpha,  /* [5 ] r_eye */
        0,   128, 128, alpha,  /* [6 ] eye_g */
        128, 128, 128, alpha,  /* [7 ] l_ear */
        64,  0,   0,   alpha,  /* [8 ] r_ear */
        192, 0,   0,   alpha,  /* [9 ] ear_r */
        64,  128, 0,   alpha,  /* [10] nose */
    ...
    for (let y = 0; y < segmap_h; y ++)
    {
        for (let x = 0; x < segmap_w; x ++)
        {
            ...
            imgbuf[idx + 0] = color[4 * val + 0];   // r
            imgbuf[idx + 1] = color[4 * val + 1];   // g
            imgbuf[idx + 2] = color[4 * val + 2];   // b
            imgbuf[idx + 3] = color[4 * val + 3];   // a
        }
    }
...
async function startWebGL()
{
    ...
    async function render (now)
    {
        ...
        /* --------------------------------------- *
         *  invoke TF.js (Face segmentation)
         * --------------------------------------- */
        let segmentations = [];
        let time_invoke1 = 0;
        for (let face_id = 0; face_id < detections.length; face_id ++)
        {
            ...
            segmentations[face_id] = await invoke_face_segmentation (feed_image);
        ...
        for (let face_id = 0; face_id < detections.length; face_id ++)
        {
            render_segment_face_image (gl, 0, 0, win_w, win_h, detections, face_id, segmentations[face_id]);
        }
