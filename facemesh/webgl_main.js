...
function startWebGL()
{
    ...
    async function render (now)
    {
        ...
        if (facemesh_ready)
        {
            ...
            {
                if (GLUtil.is_camera_ready(camtex))
                    face_predictions = await facemesh_model.estimateFaces (camtex.video);
                else
                    face_predictions = await facemesh_model.estimateFaces (imgtex.image);
            }
